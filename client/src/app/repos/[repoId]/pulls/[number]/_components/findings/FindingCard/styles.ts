...
export const s = {
  card: (focused: boolean, sevColor: string, muted: boolean): CSSProperties => ({
    borderRadius: 8,
    // All-longhand — never mix `borderColor` shorthand with `borderLeftColor`
    // longhand on the same element: React warns about shorthand/longhand conflict
    // during rerender. Use explicit per-side colors instead.
    borderStyle: "solid",
    borderWidth: 1,
    borderLeftWidth: 3,
