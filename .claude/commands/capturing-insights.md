Run the `capturing-insights` skill against the current session.

Walk through the three checkpoints from the skill:

1. **Pre-work read** — re-read every `INSIGHTS.md` for packages this session
   touched. Confirm in one line what you read AND summarize the top-3
   entries most relevant to the current task.
...
Use the entry format defined in `.claude/skills/capturing-insights/SKILL.md`.
